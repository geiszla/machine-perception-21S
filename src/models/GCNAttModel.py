...
GCN predictor and GCN autoencoder for attention network.
Inspired by https://github.com/wei-mao-2019/HisRepItself.
"""
import numpy as np
import torch
...
class GCNAttModel(BaseModel):
    """GCN human motion predictor with GCN attention network."""
    ...
    def create_model(self, in_features=135, kernel_size=17, d_model=256, num_stage=12, dct_n=34):
        ...
        self.GCEncoder = nn.Sequential(
            GCN4attn.GraphConvolution(in_features=kernel_size, out_features=7, node_n=in_features),
            GCN4attn.GraphConvolution(in_features=7, out_features=5, node_n=in_features),
        )
        self.GCDecoder = nn.Sequential(
            GCN4attn.GraphConvolution(in_features=5, out_features=7, node_n=in_features),
            GCN4attn.GraphConvolution(in_features=7, out_features=kernel_size, node_n=in_features),
        )

        self.gcn = GCN4attn.GCN(
    ...
    def forward(self, batch: AMASSBatch, output_n=24, input_n=120):
        ...
        # print(vl)
        kl = self.kernel_size
        idx_val = np.expand_dims(np.arange(vl), axis=0) + np.expand_dims(np.arange(vn), axis=1)
        src_value_tmp = src_tmp[:, idx_val].clone().reshape([bs * vn, vl, -1])
        # print('src_value_tmp :' + str(src_value_tmp.shape))
        src_value_tmp = (
            torch.matmul(dct_m[:dct_n].unsqueeze(dim=0), src_value_tmp)
        ...
        idx_keys = np.expand_dims(np.arange(kl), axis=0) + np.expand_dims(np.arange(vn), axis=1)
        ...
        score_tmp = (
            torch.matmul(key_tmp, query_tmp.transpose(2, 3)).diagonal(dim1=-2, dim2=-1) + 1e-15
        )
        ...
        out_gcn = torch.matmul(
            idct_m[:, :dct_n].unsqueeze(dim=0), dct_out_tmp[:, :, :dct_n].transpose(1, 2)
        )
        outputs.append(out_gcn)

        outputs_cat = torch.cat(outputs, dim=2)
        # print(outputs.shape)
        model_out["predictions"] = outputs_cat[:, -self.config.target_seq_len :]
        model_out["loss_AE"] = (decoded_key_tmp - src_key_tmp).pow(2).reshape(
            [bs, vn, 135, -1]
        ).sum(dim=-1).sum(dim=-1).mean(dim=1).mean(dim=0) + (decoded_query_tmp - src_query_tmp).pow(
            2
        ).squeeze().sum(
            dim=-1
        ).sum(
    ...
    def backward(self, batch: AMASSBatch, model_out):
        ...
        loss_vals = {
            ...
            "AE_loss": model_out["loss_AE"].cpu().item(),
        }
