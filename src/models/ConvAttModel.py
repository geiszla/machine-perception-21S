"""
GCN human motion predictor with convolutional attention network.

Adapted from https://github.com/wei-mao-2019/HisRepItself.
"""
...
class ConvAttModel(BaseModel):
    ...
    def create_model(self, in_features=135, kernel_size=17, d_model=256, num_stage=12, dct_n=34):
        ...
        self.convQ = nn.Sequential(
            nn.Conv1d(in_channels=in_features, out_channels=d_model, kernel_size=10, bias=False),
            nn.ReLU(),
            nn.Conv1d(in_channels=d_model, out_channels=d_model, kernel_size=8, bias=False),
            nn.ReLU(),
        )
