...
def is_valid_rotmat(rotmats, thresh=1e-6):
    """
    Check that the rotation matrices are valid, i.e. R*R' == I and det(R) == 1.
    ...
    is_orthogonal = np.all(
        np.abs(np.matmul(rotmats, rotmats_t) - eye(3, rotmats.shape[:-2])) < thresh
    )
...
def get_closest_rotmat(rotmats):
    """
    Find the rotation matrix that is closest to the inputs in terms of the Frobenius norm.

    For each input matrix it computes the SVD as R = USV' and sets R_closest = UV'.
    Additionally, it is made sure that det(R_closest) == 1.
...
def angle_diff(predictions, targets):
    """
    Compute the angular distance between the target and predicted rotations.

    We define this as the angle that is required to rotate one rotation into the other.
    This essentially computes || log(R_diff) || where R_diff is the difference rotation
    between prediction and target.

    Args
    ----
        predictions: np array of predicted joint angles represented as rotation matrices,
            i.e. in shape (..., n_joints, 3, 3)
    ...
    # convert `r` to angle-axis representation and extract the angle, which is our measure of
    # difference between the predicted and target orientations
    angles = []
...
class MetricsEngine(object):
    ...
    It keeps track of the metric values per frame, so that we can evaluate them for different
    sequence lengths. It assumes that inputs are in rotation matrix format.
    """
    ...
    def compute(self, predictions, targets, reduce_fn="mean"):
        """
        Compute the joint angle metric.

        Predictions and targets are assumed to be in rotation matrix format.

        Args
        ----
            predictions: An np array of shape (n, seq_length, n_joints*9)
            targets: An np array of the same shape as `predictions`
            reduce_fn: Which reduce function to apply to the joint dimension, if applicable.
                Choices are [mean, sum].

        Returns
        -------
            A dictionary {"joint_angle" -> values} where the values are given per batch entry and
            frame as an np array of shape (n, seq_length).
        """
        ...
        assert (
            not self._should_call_reset
        ), "you should reset the state of this class after calling `finalize`"
        dof = 9
        ...
        pred_global = local_rot_to_global(
            pred, SMPL_PARENTS, left_mult=False, rep="rot_mat"
        )  # (-1, full_n_joints, 3, 3)
        targ_global = local_rot_to_global(
            targ, SMPL_PARENTS, left_mult=False, rep="rot_mat"
        )  # (-1, full_n_joints, 3, 3)
        v = angle_diff(
            pred_global[:, select_joints], targ_global[:, select_joints]
        )  # (-1, n_joints)
    ...
    def aggregate(self, new_metrics):
        ...
        Args
        ----
            new_metrics: Dictionary of new metric values to aggregate. Each entry is expected to be
                a numpy array of shape (batch_size, seq_length).
        """
    ...
    def compute_and_aggregate(self, predictions, targets, reduce_fn="mean"):
        """
        Compute the joint angle metric values and aggregates them directly.
    ...
    def get_final_metrics(self):
        ...
        This should only be called once all the data has been processed.
