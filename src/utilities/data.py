...
class AMASSBatch(object):
    ...
    def __init__(self, seq_ids, poses):
        """
        Initialize mini-batch of `AMASSSample`s.
    ...
    def from_sample_list(samples):
        ...
        # Here we would typically need to pad the batch, but we assume at thi point windows of
        # fixed sizes are extracted from an AMASSSample, so no need for any padding.
    ...
    @property
    def seq_length(self):
        """Get the maximum sequence length of this batch (i.e. including potential padding)."""
...
class LMDBDataset(Dataset):
    ...
    def __init__(self, lmdb_path, transform, filter_seq_len=None):
        ...
        :param filter_seq_len: Minimum sequence length for samples. Any sample below (if given)
            will be rejected.
        """
    ...
    def __getitem__(self, idx):
        ...
        with self.env.begin(write=False) as txn:
            sample = AMASSSample(
                seq_id=txn.get(id_key).decode(),
                poses=np.frombuffer(txn.get(poses_key), dtype=np.float32)
                .copy()
                .reshape(-1, self.dof),
            )
