...
def sparse_to_full(joint_angles_sparse, sparse_joints_idxs, tot_nr_joints, rep="rot_mat"):
    """
    Pad the given sparse joint angles with identity elements.

    Pad the given sparse joint angles with identity elements to retrieve a full skeleton with
    `tot_nr_joints` many joints.

    Args
    ----
        joint_angles_sparse: An np array of shape (N, len(sparse_joints_idxs) * dof)
            or (N, len(sparse_joints_idxs), dof)
        sparse_joints_idxs: A list of joint indices pointing into the full skeleton given by
            range(0, tot_nr_joints)
...
def local_rot_to_global(joint_angles, parents, rep="rot_mat", left_mult=False):
    """
    Convert local rotations into global rotations by "unrolling" the kinematic chain.
    ...
    if rep == "rot_mat":
        ...
        rots = quaternion.as_rotation_matrix(
            quaternion.from_float_array(np.reshape(joint_angles, [-1, n_joints, 4]))
        )
    else:
        rots = quaternion.as_rotation_matrix(
            quaternion.from_rotation_vector(np.reshape(joint_angles, [-1, n_joints, 3]))
        )
...
class ForwardKinematics(object):
    ...
    def __init__(
        self, offsets, parents, left_mult=False, major_joints=None, norm_idx=None, no_root=True
    ):
    ...
    def fk(self, joint_angles):
        ...
        for j in range(self.n_joints):
            if self.parents[j] == -1:
                ...
                if self.left_mult:
                    positions[:, j] = (
                        np.squeeze(np.matmul(offsets[:, :, j], rotations[:, self.parents[j]]))
                        + positions[:, self.parents[j]]
                    )
                    rotations[:, j] = np.matmul(angles[:, j], rotations[:, self.parents[j]])
                else:
                    positions[:, j] = (
                        np.squeeze(np.matmul(rotations[:, self.parents[j]], offsets[:, j]))
                        + positions[:, self.parents[j]]
                    )
    ...
    def from_sparse(self, joint_angles_sparse, rep="rot_mat", return_sparse=True):
        ...
        ----
            ...
            sparse_joints_idxs: List of indices into `H36M_JOINTS` pointing out which SMPL joints
                are used in `pose_sparse`. If None defaults to `H36M_MAJOR_JOINTS`.
            rep: "rot_mat" or "quat", which representation is used for the angles in
                `joint_angles_sparse`
            return_sparse: If True it will return only the positions of the joints given in
                `sparse_joint_idxs`.

        Returns
        -------
            The joint positions as an array of shape (N, len(sparse_joint_idxs), 3) if
            `return_sparse` is True otherwise (N, H36M_NR_JOINTS, 3).
        """
...
class SMPLForwardKinematics(ForwardKinematics):
    ...
    def __init__(self):
        ...
        super(SMPLForwardKinematics, self).__init__(
            smpl_offsets, SMPL_PARENTS, norm_idx=4, left_mult=False, major_joints=SMPL_MAJOR_JOINTS
        )
