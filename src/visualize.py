...
from utilities.motion_metrics import get_closest_rotmat, is_valid_rotmat
...
class Visualizer(object):
    ...
    def __init__(self, fk_engine):
        ...
        self.is_sparse = True
        self.expected_n_input_joints = (
            len(self.fk_engine.major_joints) if self.is_sparse else self.fk_engine.n_joints
        )
    ...
    def visualize_rotmat(self, seed, prediction, target=None, title=""):
        ...
        def _to_positions(angles_):
            ...
            # Compute positions.
            if self.is_sparse:
                full_seq_pos = self.fk_engine.from_sparse(
                    full_seq, return_sparse=False
                )  # (N, full_n_joints, 3)
            else:
...
def visualize_positions(
    ...
    """
    Visualize motion given 3D positions.

    Can visualize several motions side by side. If the
    sequence lengths don't match, all animations are displayed until the shortest sequence length.

    Args
    ----
        positions: a list of np arrays in shape (seq_length, n_joints, 3) giving the 3D positions
            per joint and frame
    ...
    def update_frame(num, positions, lines, parents, colors):
        for index in range(len(positions)):
            ...
            for i in range(1, len(parents)):
                ...
                if (
                    change_color_after_frame
                    and change_color_after_frame[index]
                    and num >= change_color_after_frame[index]
                ):
                    points_j[k].set_color(_colors[2])
                else:
                    points_j[k].set_color(colors[index])
