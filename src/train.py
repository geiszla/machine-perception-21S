...
def _evaluate(net, data_loader, metrics_engine):
    ...
    This computes the loss, but does not do any backpropagation or gradient update.
    ...
    with torch.no_grad():
        for abatch in data_loader:
            ...
            # Accumulate the loss and multiply with the batch size (because the last batch might
            # have different size).
            for k in loss_vals:
...
def main(config):
    ...
    # For the training data we pass in the `window_size` variable, so that all samples whose
    # length is smaller than `window_size` are rejected.
    train_data = LMDBDataset(
        os.path.join(C.DATA_DIR, "training"), transform=train_transform, filter_seq_len=window_size
    )
    ...
    for epoch in range(config.n_epochs):
        ...
        for i, abatch in enumerate(train_loader):
            ...
            if global_step % (config.print_every - 1) == 0:
                loss_string = " ".join(
                    ["{}: {:.6f}".format(k, train_losses[k]) for k in train_losses]
                )
                print(
                    "[TRAIN {:0>5d} | {:0>3d}] {} elapsed: {:.3f} secs".format(
            ...
            if (global_step % (config.eval_every - 1) == 0) & (not config.train_on_val):
                ...
                # Log to console.
                loss_string = " ".join(
                    ["{}: {:.6f}".format(k, valid_losses[k]) for k in valid_losses]
                )
                print(
                    "[VALID {:0>5d} | {:0>3d}] {} elapsed: {:.3f} secs".format(
                        i + 1, epoch + 1, loss_string, elapsed
                    )
                )
                print(
                    "[VALID {:0>5d} | {:0>3d}] {}".format(
                        i + 1, epoch + 1, me.get_summary_string(valid_metrics)
                    )
                )
